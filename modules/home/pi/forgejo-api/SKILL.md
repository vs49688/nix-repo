...
The API token lives at `~/.config/sops-nix/secrets/agents/forgejo_token` and goes
in an `Authorization` header as `token <token>`:
...
Sanity-check it against `GET /user`: `200` with `{"login":"<account>"}` means
the token works, `401` means it does not.
...
Milestone objects are already small (~200 B) — no need to project those.
...
- Write endpoints (`POST`/`PATCH`/`PUT` with a body) need `Content-Type: application/json`, or Forgejo returns 422 (`Empty Content-Type` or `Unsupported Content-Type`).
...
The swagger spec is ~850 KB — never pull it into context raw. It needs no auth,
and this is the one case where fetching to a file and grepping beats
`web_request`:
